    ...
    <Teleport to="body">
        <div v-if="visible" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50"
            ...
            <div class="bg-white w-1/3 max-h-[500px] p-6 rounded-lg shadow-lg relative overflow-hidden flex flex-col">
                ...
                <!-- 팝업 타이틀 -->
                <h2 class="text-xl font-bold text-center mb-4">수·배전반/신재생 에너지 편집</h2>
                <!-- 본문: 이미지 업로드, 설명 입력 -->
...
const powerSystemsData = reactive({
    description: "",
    imageUrl: "",
});
const file = ref(null);
...
// 부모로부터 전달받은 existingData가 변경되면 local state 업데이트
watch(
    () => props.existingData,
    (newVal) => {
        if (newVal) {
            powerSystemsData.description = newVal.description || "";
            powerSystemsData.imageUrl = newVal.imageUrl || "";
            // 만약 DB의 imageUrl이 "/uploads/xxx.jpg"라면 최종 URL은 "http://localhost:8080/uploads/xxx.jpg"
...
async function savePowerSystems() {
    const formData = new FormData();
    formData.append("description", powerSystemsData.description || "");
    if (file.value) {
        formData.append("file", file.value);
    }
    try {
        const response = await axios.post("http://localhost:8080/api/business/power-systems/save", formData, {
            headers: { "Content-Type": "multipart/form-data" },
        });
        console.log("savePowerSystems => response:", response.data);
        alert("저장 완료!");
        emit("close");
    } catch (error) {
