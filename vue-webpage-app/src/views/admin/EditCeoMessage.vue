<template>
    <div class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
        <div class="bg-white w-1/3 max-h-[500px] p-6 rounded-lg shadow-lg relative overflow-hidden flex flex-col">
            ...
            <h2 class="text-xl font-bold text-center mb-4">CEO 인사말 편집</h2>
            ...
            <div class="flex-grow overflow-y-auto px-2">
                ...
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">CEO 메시지</label>
                    <textarea v-model="ceoMessage.message" rows="4"
                        class="w-full border p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"></textarea>
                </div>
            </div>

            <div class="flex justify-end mt-4">
                <button @click="saveCeoMessage"
...
export default {
    ...
    methods: {
        ...
        async saveCeoMessage() {
            const formData = new FormData();
            formData.append("message", this.ceoMessage.message || "");
            ...
            try {
                await axios.post("http://localhost:8080/api/ceo-message/save", formData, {
